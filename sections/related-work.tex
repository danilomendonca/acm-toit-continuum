...
Recently, several works have tackled the problem of placing application components and computation offloading among computational entities from cloud and edge computing. Due to its combinatorial nature, this placement problem is NP-Hard. Each application components may be placed on different entities, but the model does not cope with distributed instances of the same component. 

In this category, Want et al.~\cite{} proposed an online placement approximation of multi-component applications as a graph placement problem. The proposal considers computational resources of physical nodes and the capacity of network links. 

In contrast, the continuum model enables $\mu$-service instances to co-exist across the continuum and verses about the opportunistic allocation of instances within a domain boundary. 
