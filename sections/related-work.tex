...
A number of works have tackled the problem of placement among the set of computational entities from edge and cloud computing. Some (e.g., ~\cite{Tarneberg2017}) focus on the placement of single components, whereas others (e.g., ~\cite{Wang:2017}) consider multi-component applications. In a similar context, Wang et al.~\cite{Wang:2015} proposed a service migration algorithm that takes user mobility into account. More closely related to our service model, Jia et al.~\cite{Jia:2017} tackled the offloading of virtualized network functions (NVF) to distributed cloudlets. 
