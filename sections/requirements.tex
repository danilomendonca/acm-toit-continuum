...
\item  Agnostic with respect to the capabilities and specific technology details of different computing platforms (\stepcounter{req_count}\textbf{R\arabic{req_count}}); and

\item Flexible, so that different instances of the model can address the particularities of distinct types of computing platforms (\stepcounter{req_count}\textbf{R\arabic{req_count}}).
...
\begin{figure}[tbp]
	...
	\null\hfill
	\subfloat[Heterogeneus and independent types of edge computing infrastructure (mobile and local) in range of communication with a client device\label{fig:edge-heterogeneity}]{ \includegraphics[width=0.5\textwidth]{figs/edge-heterogeneity.png}}
