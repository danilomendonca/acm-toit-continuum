...
The continuum encompasses distinct types of computational resources located at the cloud and at the edge of the network (Fig.~\ref{fig:edge-heterogeneity}). Accordingly:

\begin{itemize}

\item the unified model should be: 1) agnostic with respect to the capabilities and specific technology details of different computing platforms (\stepcounter{req_count}\textbf{R\arabic{req_count}}); and

\item 2) flexible, so that different instances of the model can address the particularities of distinct types of computing platforms (\stepcounter{req_count}\textbf{R\arabic{req_count}}).
...
\begin{figure}[tbp]
	...
	\null\hfill
	\subfloat[Heterogeneus and indpendent types of edge computing infrastructure (mobile and local) in range of communication with a client device\label{fig:edge-heterogeneity}]{ \includegraphics[width=0.5\textwidth]{figs/edge-heterogeneity.png}}
