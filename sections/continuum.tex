...
After work, our user enters her autonomous vehicle (AV). During her way home, the vehicle makes use of services 
%deployed at mobile-edge servers located at cellular base stations 
to receive updates about the best plan to reach her destination. Additionally, it must rely on low-latency services to become aware of sudden events like accidents.

To support this application scenario, two continuum $\mu$-services are envisioned: the \textit{route planning}, which relies on a traffic dataset and a routing software and can cope with higher latency, and the \textit{incident notification}, which relies on low-latency communication between vehicles and mobile-edge base stations and among nearby stations.
...
%TODO remove after per application discussion
