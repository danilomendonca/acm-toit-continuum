...
Mobile devices (i.e., User Equipment~\cite{etsimec16}) play two roles, namely clients and potential providers of continuum services. The motivation for including mobile devices' own virtualized resources in the model is threefold: (i) the substantial increase in computational capacity of modern devices; (ii) the compatibility of these devices to transparently execute services (see Section~\ref{sec:application_model}); and (iii) to give mobile applications a zero network latency and highly available alternative to cloud and edge providers. 
...
A domain yields a common  abstraction (typically through virtualization) for the heterogeneous compute, storage and network resources (e.g., server(s), virtual machine(s), container(s), memory, CPU, storage, etc.) composing the continuum\footnote{The definition for \textit{domain} extends the one provided by ETSI for a \textit{mobile edge host}~\cite{etsimec16} to the continuum.}. 
...
The management of continuum services life-cycle is focused on two conflicting aspects: i) satisfy application rules and requirements (e.g., maximum service latency, battery consumption, availability~\cite{etsimec16}); and ii) optimize resources employed by service providers. 

From the providers' perspective, an optimal allocation of its domain (virtualized) resources corresponds to the minimum number of $\mu$-service instances that must be allocated by each of its domains to cope with the SLA of each provided service.% (e.g., maximum service latency, simultaneous request threshold, etc). 
