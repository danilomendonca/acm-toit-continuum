...
From the providers' perspective, an optimal allocation of its domain resources corresponds to the minimum number of continuum $\mu$-service instances that must be allocated by each of its domains to cope with the service-level agreement (SLA) of each provided service.% (e.g., maximum service latency, simultaneous request threshold, etc). 

For instance, let's consider the set of $\mu$-services $s_i \in S$, with $1 \le i \le m$ and $m = |S|$. Also, each instance $f_{ij} \in F_i$ from $s_i$ is bound to a single core container with fixed memory and storage. Considering that CPU is a scarcer resource, we may assume, without loss of generality, that the number of instances per domain is limited by the amount of CPU cores available ($|C|$) in its pool of resources, i.e., that $\sum_{1}^{j} |F_i| \le |C|$.

Now let's consider that each provided $\mu$-service $s_i \in S$ is bound to a SLA specifying the maximum service delay $\Delta_i$ and the maximum number of instances $N_i$, so that $\forall F_i, |F_i| \le N_i$. %, and a relative priority $p_i$. 
...
From the client's viewpoint, an optimal provider selection corresponds to the one that, given a set of requirements, satisfies the multi-objective optimization.

For instance, if we consider service latency and cost as our requirements...

TODO: formalize it here
