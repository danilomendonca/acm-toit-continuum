...
%Client requests may originate from different geographic locations. Edge nodes may or not be available at that specific area. The demand for different services originated at each location is dynamic due to the variations in application usage and due to the mobility of clients. 
...
Service requests may originate from different parts of the network. The demand for services coming from each location is dynamic due to variations in application usage and the mobility of clients. 

With the aim of optimizing the usage of computational resources, continuum microservices should be instantiated on demand. 

%and where needed, i.e., just-in-time and by the domains best satisfying latency and other application requirements. %TODO domain's cost?    

This is aligned with the \textit{Function-as-a-Service} (FaaS) execution model~\cite{MateosFaaster17}. FaaS has been proposed as an alternative cloud paradigm in which business functionality is provided without pre-allocating computational resources. Instead, shared resources (e.g. containers) are used to provision and execute functions on demand, in only a few milliseconds. In the FaaS model functions are ephemeral (they may even last for only one invocation), stateless, and scaled as needed~\cite{Roberts:2016}. 



Two types of requirements are considered: location and QoS. The first allows the client to fix the type of domain that should serve a request. The latter specifies the QoS levels to be achieved by eligible domains.

The total service latency can be refined by three components: the \textit{network delay} between the client device and the physical node hosting the service execution; the \textit{setup delay} (or \textit{cold start}), which comprises the elapsed time for a service instance to be up and running at a given node; and finally the \textit{computation delay}. 

Network delay should be minimized with the placement of service instances to the nearest edge-domain~\footnote{the precise latency of surrounding domains must be monitored} or nullified if the mobile-domain can afford the resources. Whenever possible, setup delay should be minimized with the anticipation of requests. 
...
%The FaaS execution model also favors portability among domains, as similar functions~\footnote{Due to its limitations, mobile domains may not be suitable for executing certain functions.} may be executed by a mobile domain's runtime (e.g., a Java runtime in an Android platform) or by dynamically created runtimes provided by cloud- and edge-based~\cite{GarrigaMendonca2017} FaaS platforms (e.g., a Java runtime provided by Apache OpenWhisk~\footnote{OpenWhisk is a well known open source serverless platform -- \url{http://openwhisk.incubator.apache.org/}} or AWS Lambda).
