...
To better express this problem, let's consider the set of $\mu$-services $s_i \in S$, with $1 \le i \le m$ and $m = |S|$. Each $s_i \in S$ has a corresponding set of instances $f_{ij} \in F_i$, with $0 \le j \le k_i$ and $k_i = |F_i|$. Now let's consider that each provided $\mu$-service $s_i \in S$ is bound to a SLA specifying the maximum service delay $\Delta_i$ and the maximum/minimum number of instances $Kmax_{i}/Kmin_{i}$, so that $\forall F_i, Kmin{i} \le k_i \le Kmax_{i}$. %, and a relative priority $p_i$. 
Given the current demand and delay (respectively $u_i$ and $\delta_i$) associated to each service $s_i \in S$, the aim of the \textit{domain manager} is to assure that $\forall s_i \in S, \delta_i \le \Delta_i$. 
