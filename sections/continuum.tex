...
From the continuum provider's perspective, an optimal allocation of resources corresponds to the minimum number of $\mu$-service instances that must be allocated by each of its domains to cope with the service-level agreement (SLA) of each provided service (e.g., maximum service latency and availability). 
...
The life-cycle management problem is therefore dual: the provider-side allocation of service instances at each domain; and the client-side decision of which domain must be selected for each continuum $\mu$-service.
