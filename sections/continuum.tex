...
In the continuum, a plethora of disjoint cloud and edge domains can host the execution of $\mu$-services, i.e., they must be able to handle operational aspects such as \textit{downloading and installing} the $\mu$-services. Mobile clients will freely enter and exit geographical areas that are covered by distinct edge domains. In fact, even cloud domains may see considerable variations to their aggregate demand over time. In such a scenario it may be unfeasible to predict the origin and intensity of the demand that the different $\mu$-services at each domain might see. 
