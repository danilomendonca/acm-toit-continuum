...
In our formulation, edge nodes are distinguished after the networking technology used: mobile-edge (also known as Multi-Access Edge Computing or MEC~\cite{ahmed2016isco}) integrates with cellular network infrastructure (e.g., 5G base station), whereas local-edge integrates with local area network infrastructure (e.g., a wireless access point). Also, we assume that cloud and edge providers may be different and that communication among heterogeneous edge nodes may not be feasible..% and unable to coordinate the allocation of resources.
...
Figure X illustrates a topology composed of cloud, mobile-edge, local-edge, and mobile \textit{domains}. 
