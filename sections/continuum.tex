...
Now let's consider that each provided $\mu$-service $s_i \in S$ is bound to a SLA specifying the maximum service delay $\Delta_i$ and the maximum/minimum number of instances $Kmax_{i}/Kmin_{i}$, so that $\forall F_i, Kmin{i} \le k_i \le Kmax_{i}$. %, and a relative priority $p_i$. 
