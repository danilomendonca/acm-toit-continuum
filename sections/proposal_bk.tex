...
%A3-E addresses all actors in the compute continuum formed by mobile devices, edge, and cloud servers. 
...
Thus, from the domain side, the lack of awareness of connected clients prevents triggering the acquisition and subsequently allocation phases based on the detection of a client. From the client side, the lack of awareness from surrounding domains prevents clients of deciding which alternative domains must be used. %I'm not sure if I understand this paragraph (Martin)

\subsubsection{Acquisition Phase}\label{sec:A3-E-acquisition}

In turn, the Acquisition phase has the following purposes:
...
From the domain-side, the use of a specific policy may depend on factors like:

\begin{itemize}

\item The type of domain (e.g., cloud, mobile-edge, local-edge); and

\item The type of application and the SLA between client and provider;

\end{itemize}

Above all, the choice of a policy by a domain is affected by two conflicting properties: 1) the application tolerance for service \textit{setup delay}; and 2) the \textit{efficiency} of domain resource usage. Eq.~\ref{eq:setup_delay} models the total delay of service setup:
...
\noindent
In which the first term ($C_{OFFLINE}$) represents the resources required to download and install the services (e.g., network, storage) and the later ($C_{RUNTIME}$) represents the resources needed for executing the services (e.g., memory and CPU).

Considering the $FRA$ from a client application as the reference event, the more \textit{reactive} the policies are to that event, the more delay the first request is served with. Conversely, the more \textit{pro-actively} services are made ready for execution, the less delay the first request to each of these services are served with. 

For example, if both acquisition and allocation of services to the domain are reactive, 

From the efficiency perspective, the relation is the opposite: the more \textit{reactive} the policies are with respect to the $FRA$ event, the lower the periods of underutilization and idleness of the domain resources. In contrast, the chances of underutilization and idleness are higher with \textit{proactive} policies. 

The domain-side policies in Fig.~\ref{fig:A3-E-domain} are:
...
--- \textbf{Mobile}: this instance represents the local computation performed by mobile devices. It provides a zero network latency with high availability (functions are installed and allocated with the rest of the application). In contrast, the resource constraints of mobile devices may result in lower processing performance and undesirable battery drain, therefore justifying the use of other instances (mobile computation offloading), unless none is available. Targeted services: lightweight and latency-sensitive computation.



%Notwithstanding this, in this paper we focus on the alternatives described by lines 3 and 4 of Table~\ref{tab:A3-E-instances}, as they represent suitable alternatives for mobile and local types of edge computing.
