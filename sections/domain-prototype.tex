...
The domain-side middleware prototype described herein focuses on a local-edge domain, and includes a complete realization of the Advertisement, Acquisition and Allocation phases of A3-E -- as depicted in Fig.~\ref{fig:local-edge-domain-prototype}. The Engagement phase is not covered by the local-edge DSM since it occurs between the client application (through the client-side middleware) and the FaaS platform (OpenWhisk). The prototype was implemented using Python\footnote{Documentation and source code are available at \url{https://github.com/deib-polimi/A3-E-DSM/}}, and runs within a Linux-based domain hosting also OpenWhisk. 
