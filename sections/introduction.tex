...
--- Last but not least, edge computing should complement rather than replace the cloud-based services. In this sense, edge computing should be seen as part of a continuum with the cloud in which opportunistic edge services are backed by cloud counterparts with high availability. Additionally, given the physical proximity between mobile clients and edge servers, the offloading of delay-sensitive computation from resource constrained devices to edge servers becomes possible. In this case, the opportunistic nature of edge services should be backed by the high availability of local computation. Together, cloud, edge, and mobile should form what we call a \textit{computational continuum}.
