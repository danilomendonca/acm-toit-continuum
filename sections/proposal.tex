...
The \textit{engagement} phase models the actual interaction between a client and a service. At this stage, the requested service must be already allocated by the domain. For instance, traditional cloud-based services are pre-allocated and engage with clients by means of a request/response protocol. In the later case, the remaining A3-3 phases are not employed. Nonetheless, in other scenarios of the continuum --- specially those with resource limitations --- a dynamic and opportunistic allocation shall improve the efficiency in which domain resources are consumed.
...
From the domain perspective, A3-E addresses the needs of resource-constrained domains in the continuum by adhering to the FaaS execution model in which stateless functions can be deployed and scaled on demand without minimum pre-allocation of resources. In particular, the allocation phase is modeled by a self-management loop~\ref{} in which resources are allocated and deallocated according to: 1) the SLA between the domain and different applications; 2) the allocation policy employed by the domain. For instance....

%More recently, the FaaS model introduced zero-allocation mechanism, in which stateless functions may remain deallocated until they are invoked (cold start). Once the demand increases, the FaaS platform may respond with additional instances of the required functions. 

From the client perspective, the allocation phase refers to the selection, at a specific context, of the domain that best suits its requirements from a list of available domains and respective quality-of-service (QoS) attributes. Such list and properties must be monitored as part of a self-management loop (Fig.~\ref{}) that checks for the availability of listed domains and the QoS levels of each available domain. For instance....



%adheres to the dynamic allocation of computational resources for services execution. Applications that can not afford the delay (hereafter named $\Delta_{AL}$) of a cold start must rely on services that are always kept available with minimum allocation. Instead, whenever cold start is acceptable, the zero-allocation provides a more efficient usage of computational resources. Needless to say, $\Delta_{AL}$ is only perceived by the first request after a cold start, while subsequent calls are processed normally hereafter.
