...
A3-E targets the efficient and scalable placement of $\mu$-services along the continuum. For this, clients and distinct providers (domains) take part in the \textit{automated}, \textit{decentralized}, and \textit{dynamic} decision of which continuum resources --- among those of mobile, edge, and cloud --- should be employed in the provisioning of each $\mu$-services required by continuum applications.
...
Remote domains (i.e., cloud and edge) are engaged through distributed protocols (e.g., through HTTP requests or WebSockets). To enforce a common interface between the client middleware and heterogeneous domains, the mobile domain should also expose its functionality as local services decoupled from the client application.
...
\begin{itemize}
	...
	\item \textbf{Request arrived:} indicates the arrival of a request for a specific $\mu$-service. The manager must handle the event with the allocation of the request to an service instance. 
	%TODO [Danilo] Sounds like load balancing; is it so?
	%TODO: [Gio]  shouldn't it be "Service requested"?
...
\begin{figure}[tbp]
	...
	\caption{Dynamic resource allocation based on control-theory.}
...
The \textit{Allocation} models the placement of functions and dependencies on the cloud and edge domain's resources, i.e., on its pool of servers, virtual machines, and containers. It also captures the client-side selection of the provider domain for each $\mu$-service employed by the application.
...
\subsubsection*{Domain-side Allocation}

The scope of the domain-side allocation is limited by the domain's boundaries. For example, cloud domains allocate functions to containers in resourceful datacenters covering a large area. On the other hand, edge domains rely on containers from one or more servers covering a building or an office (local-edge) or a 5G base station area (mobile-edge).
...
\begin{itemize}
	...
	\item \textbf{Service-requested} as defined in Section~\ref{sec:A3-E-engagement}. 
	
\end{itemize}

To realize this activity, the domain manager exploit a self-management control loop~\cite{kephart2003vision} to dynamically allocate resources to each $\mu$-service according to their needs. In general, a service provider has to guarantee a QoS defined in an SLA. 
To cope with low-latency nature of the continuum applications and the highly-varying workload of service requests incoming from users (i.e., users that quickly enter and leave a particular edge area) we i) consider the \textit{response time} as the reference metric of QoS and ii) propose a highly scalable and fast implementation of the self-management loop based on control-theory~\cite{Quatrocchi2016discrete}. 

Figure~\ref{fig:A3Edomain-manager} shows an high-level view of our control system. The system to be controlled is the provider's resource pool composed by a cluster of (virtual) machines. In those machines containers are deployed and execute instances of $\mu$-services. The system, or plant in control-theoretical terms, is subject to different signals that could be measurable (input variables) or unknown (disturbances). Considering a discrete time, for each $\mu$-services we define as $\lambda(k)$ the measured arrival rate of request at time $k$ and the vector containing this values as $\bar{\lambda}(k)$ for each control interval. At time $k$ each $\mu$-services is executed in a $c(k)$ number of containers or replicas and $\bar{c}(k)$ the corresponding vector. The size of the container is considered fixed coherently to the architecture of serverless framework such as OpenWhisk. For completion the disturbances are defined as $\bar{d}$ and by definition cannot be controlled. 

The output variable of the system is the vector of response times produced by each $\mu$-service modeled as $\bar{\tau}$ while we define the desired response times or set-point as $\bar{\tau}^\circ$. We dedicate one controller per service with the goal to compute the next container allocation for the service in order to obtain:

\begin{equation}
	\tau(k)^\circ - \tau(k) = e(k) = 0\quad \forall{k} > 0
\end{equation}
...
To do that the controller must model the system with enough details to govern the dynamics, this in control theoretical is done by defining a characteristic function for the system. This function does not need to be linear but regular enough be linearizable in the domain space of interest. Therefore we assume that response time depends on the ratio between the number of allocated container and the request rate.  This function, $f$ to give it a name, is intuitively monotonically decreasing towards a possible lower horizontal asymptote, as it can be assumed that once the parallelism degree of an $\mu$-service is fulfilled by the available containers, adding new ones causes no further decrease in the response time. More specifically, we found a practically acceptable function to be

\begin{equation}
f \left( \frac{c(k)}{r(k)}\right) = c_1+\frac{c_2}{1+c_3\frac{c(k)}{r(k)}} = \widetilde{u}(k)
\label{eqn:CsysModel-f}
\end{equation}
\noindent where parameters $c_1$, $c_2$, and $c_3$ were obtained through profiling of each $\mu$-service and thus we obtain the following dynamic system:
...
where $p$ is the single pole of the system (the closer to $1$ the more conservative will be the control). As control techniques we rely on PI controllers since the system belongs to the first order.  %TODO: cite 

At each control step the controllers compute the next container allocation for the relative $\mu$-service forming the $\bar{c}$ vector. This allocations are not immediately actuated (through the container manager of choice such as Docker Swarm or Kubernetes) since the sum of the allocations could be greater than the entire capacity of the resource pool. In fact $\bar{c}$  is passed to another control component called  \textit{ContentionManager}. The  output of ContentionManager is a vector $\bar{c}'$, the actual container allocation $\mu$-services, with 
\begin{equation}
		\bar{c}'(k) =
	\begin{cases}
			\bar{c}(k),& \text{if no resource contention}\\
			scaleDown(\bar{c}(k)),              & \text{otherwise}
	\end{cases}
...
In the scenario introduced in Section~\ref{sub:example}, the AV application should have a higher priority with respect to non-critical applications such as AR applications for tourists~\cite{GarrigaMendonca2017}. In case of contention the edge $\mu$-services might become unavailable to the AR applications, for example during rush hour. In that case the AR and other low-priority applications would have to rely on $\mu$-services being run in the mobile domain, or in a cloud domain.
