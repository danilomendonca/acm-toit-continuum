...
\begin{figure}[tbp]
	\includegraphics[width=0.85\textwidth]{figs/A3-E-model.pdf}
...
Above all, the use of specific policies affect the following (conflicting) properties: a) the \textit{efficiency} of domain resource usage (domain side); and b) the application tolerance for service setup \textit{delay} (client side). Considering the first request from a client application as the reference event, the more \textit{reactive} the policies are to that event, the less time domain resources are likely to remain idle before it happens. In contrast, the chances of underutilization and idleness are higher with \textit{proactive} policies. Eq.~\ref{eq:setup_cost} models the total delay of service setup:
...
From the delay point of view, the relation is the opposite: the more \textit{reactive} the policies are with respect to the first request arrival event, the higher the delay the first request to each service is served with. In the other direction, the more \textit{pro-actively} services are made ready for execution, the less delay the first request to each of these services are served with. Eq.~\ref{eq:setup_delay} models the total delay of service setup:
...
Thus, the use of a specific policy to be employed by a domain depends on the availability of resources at a domain, the delay requirements of the application; and the SLA between the two parts. In specific, the domain-side policies in Fig.~\ref{fig:A3-E-domain} 
