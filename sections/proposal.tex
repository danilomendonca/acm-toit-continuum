...
in which $\mu$-service instances (i.e., $c_j$) are allocated to cloud and edge domain resources to guarantee that service latency (i.e., $\tau_j$) and availability of each provided $\mu$-service $s_{p,j} \in S_p$ are in-line with the desired SLA. 

To achieve this goal, the self-management loop takes into account the \textit{arrival rate} and the \textit{number of clients} --- respectively indicated by a \textit{$\mu$-service requested} and a \textit{client arrived} events. Also, to prevent the \textit{cold start}
of new or deallocated $\mu$-services, 
the manager reacts to a 
a \textit{$\mu$-service acquired} event by anticipating Allocation, if resources are available.
...
From the client's viewpoint, the fluctuations in the availability and service latency must be handled with the dynamic selection of the domain alternative 
that best satisfies the client requirements. 
Analogously to the provider-side Allocation, the mobile middleware realizes this activity by means of a self-management loop~\cite{kephart2003vision} for each $\mu$-service consumed by the client application.

Throughout each selection loop, the middleware handles a \textit{domain acquired (lost)} event --- indicating the agreement (denial) of a domain in providing a given $\mu$-service --- by including (disregarding) such domain. Additionally, the mobile middleware \textit{monitors} the actual service latency ($actual_{p,a,u}$) from the list of \textit{capable domains} ($D_c \subset D_p$), as well as its own \textit{battery level}. Then, given the outcome of a multi-attribute analysis encompassing \textit{service latency} and \textit{battery consumption}, at each loop iteration the middleware decides for keeping the current domain or triggering a \textit{domain change} event in case a better alternative is found. 
...
Figure~\ref{fig:A3-E-instance-events} depicts a timeline of events from our Running Example scenario. The timeline begins with our user starting the AR after entering the touristic. The mobile middleware receives a \textit{domain identification} ($DI$) from its mobile domain, to be replied with a \textit{client identification} ($CI$). Following the $\mu$-service identification ($\mu I$), the MD triggers a \textit{$\mu$-service allocated} ($\mu AL$) once the corresponding functions have been registered. After selecting this domain, the MM triggers a \textit{domain changed} ($DC$), which allows subsequent \textit{C-request arrived} ($CR$'') to be handled locally. As our user's device enters a base station area (BSA) featuring a mobile-edge domain, the middleware receives a \textit{domain identification} ($DI$) and repeats the previous handshake procedure. To prevent battery drain, the self-management loop decides for the mobile-edge, triggering a $DC$ once the $\mu AL$ signal arrives. After a period of engagement, our user enters the AV featuring a local-edge domain and, upon mutual awareness, switches to that domain ($DC$) as its connection with the mobile-edge is lost ($DL$). Since it is a first contact with an AR application, the AV's local-edge must perform Acquisition, whose completion is indicated by a \textit{$\mu$-service acquired} ($\mu A$') --- to prevent service interruption, the mobile domain is momentarily selected until a $\mu A$'' signal arrives from the local-edge.
