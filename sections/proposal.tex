...
In the A3-E model, a \textit{service} is a set of artifacts that fulfill certain functionality and can be deployed to and accessed from a remote server. In particular, service artifacts can refer to: (i) the actual function(s) exposed to the clients (e.g., a Java compiled class); (ii) static data (e.g., a trained neural network model); and (iii) other dependencies (e.g.,  software libraries). Moreover, these services can be considered as \textit{microservices}~\cite{lewis2014microservices} as they are small, modular, communicate with lightweight mechanisms (often an HTTP RESTful API) and are independently deployable by fully automated deployment machinery.

Services are provided through \textit{domain}s. A domain provides a common abstraction for the constituents of the continuum. Indeed, it hides the fact that they make use of heterogeneous \textit{computational resources} (e.g., server(s), virtual machine(s), container(s), memory, CPU, storage, etc.) and networking infrastructures (e.g., access points, radio access networks, etc.). Examples of these domains are \textit{cloud-domains}, i.e., cloud-based FaaS platforms covering a specific geographical region (e.g., Amazon Lambda in Europe); \textit{edge-domains}, representing local-edge and mobile-edge sites (e.g., servers within an office building or at cellular base stations); and \textit{mobile-domain}, representing a client device's own resources.
...
A3-E exploits the Serverless Architecture~\cite{baldini2017serverless} to guarantee that the heterogeneous domains in the continuum are efficient, scalable and automatable.~\cite{GarrigaMendonca2017}~\cite{Hendrickson:2016}).
...
client-side logic (if any) and user interfaces. Server-side logic (if any) and persistence (if any) are provided by conventional cloud services. Finally, the application relies on the continuum for stateless computation, which is provided by microservices implemented as stateless function(s) and, if any, static dependencies. 
...
Considering our running example, the client-side mobile game application consists of controllers and user interfaces. Microservices in the continuum provide the functionality for processing and updating the game state, which must be passed as parameter in conjunction with game events (e.g., user inputs). Finally, it also includes cloud services providing stateful business logic (e.g. authentication) and persistence (e.g., players scores). 

Also in the running example, the client-side AR application captures scenes from the device's camera and rely on the continuum microservices to extract features from objects in the scene (e.g., the user's desk, hands, crafting tools) based on a static trained model. It then relies on client-side logic to render the updated 3D virtual object in the scene. In this case, conventional cloud services are not employed.
...
%To enforce a common interface between clients and heterogeneous providers, mobile devices should also expose offered functionality as local services.
