...
The acquisition phase is realized by the DSM with the following sub-process: First, receiving from the CSM the set of application requirements, namely the services to be installed along with a repository URL from which their artifacts can be downloaded. Then, throughout the services life-cycle, the DSM also checks for new versions of acquired services from the same repositories and updates them accordingly. Finally, whenever service acquisition finishes, the DSM informs their clients about its (un)availability.
