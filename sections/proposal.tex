...
Throughout provider-side Allocation, a \textit{$\mu$-service acquired} event indicates that the function(s) and dependencies have been fetched, and that the $\mu$-service is ready to be deployed. The domain manager must react by including this $\mu$-service in its self-management loop. \textbf{THIS SELF-MANAGEMENT LOOP IS NOT WELL INTRODUCED. IT KIND OF DROPS OUT OF NOWHERE} To mitigate cold starts, the manager should react to a \textit{client arrived} event by anticipating Allocation, if resources are available. 

Finally, a \textit{$\mu$-service requested} event related should be taken into account by by the self-management loop handling Allocation with the increase of the corresponding arrival rate. \textbf{I DON"T UNDERSTAND WHAT YOU MEAN HERE.}
