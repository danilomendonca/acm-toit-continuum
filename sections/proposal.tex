...
\item \textbf{Proactive}: allocation phase starts upon external event preceding the arrival of the first request (e.g., triggered by a prediction of service usage); $FR_D$ does not include $\Delta_{AL}$;

\item \textbf{Sequential}: allocation phase starts as soon as acquisition phase finishes; $FR_D$ may include a fraction of $\Delta_{AL}$ if $FRA$ precedes the end of allocation; or

\item \textbf{Reactive}: allocation phase starts as soon as the first request for a service arrives; $FR_D$ includes $\Delta_{AL}$ (also known as \textit{cold start}).
