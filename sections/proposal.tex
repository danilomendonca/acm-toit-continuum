...
Fig.~\ref{fig:Continuum-arch} illustrates the architecture of a mobile application designed to work with both the continuum and conventional cloud computing. The client-side application consists of (client-side) business logic and user interfaces. Stateful server-side business logic (if any) and persistence (if any) are provided by conventional cloud services. Finally, the application relies on the continuum for stateless computation, which is provided by microservices implemented as stateless function(s) and static dependencies (if any).
