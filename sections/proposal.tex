...
Given the resource limitations of edge domains, contention between different $\mu$-services may exist. For instance, if we go back to our running example in Section~\ref{sub:example}, critical applications should have a higher priority with respect to others like the AR for tourists~\cite{GarrigaMendonca2017}. In case of contention, edge-based $\mu$-services might become unavailable (or available with an higher response time) to the AR applications. In that case the AR and other low-priority applications could have to rely on $\mu$-services from their own mobile domain or a cloud domain --- to be decided by the client-side Allocation.
