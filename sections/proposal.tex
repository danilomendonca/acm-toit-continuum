...
by including (disregarding) such domain. At each loop iteration, the mobile middleware \textit{monitors} the \textit{network latency} from the list of \textit{capable domains} ($D_{j} \subset D$), as well as its \textit{battery level}. It then \textit{analyzes} the actual service latency by also taking into account the computational power of each domain, as well the \textit{battery consumption}. Then, given the outcome of a multi-attribute rating encompassing \textit{service latency} and \textit{battery consumption}, the middleware decides for either keeping the current domain or triggering a \textit{domain selected} event in case a better alternative is found. A detailed implementation of the client-side self-management loop can be found in Sec.~\ref{sec:implementation}.
