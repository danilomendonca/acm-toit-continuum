...
in which $\mu$-service instances ($c_j$) are allocated to cloud and edge domain resources to guarantee that service latency ($\tau_j$) and availability of each provided $\mu$-service $s_{p,j} \in S_p$ are in-line with the desired SLA. 

To achieve this goal, the self-management loop takes into account the \textit{arrival rate} (vector X), the \textit{number of clients} (vector Y), and the min/max number of instances $Kmin_{j}/Kmax_{j}$ from each $s_{a,i} \in S_a$. The first parameter is updated by \textit{$\mu$-service requested} events from Engagement, whilst the second depends on \textit{client arrived} and \textit{client left} events from Awareness. Finally, to prevent the \textit{cold start} of new or deallocated $\mu$-services, the manager reacts to a \textit{$\mu$-service acquired} event by anticipating Allocation, if resources are available at that time. 
