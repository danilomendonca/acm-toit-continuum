...
As state is a fundamental aspect of most applications, we argue that mobile devices and, for most cases, cloud datacenters should remain responsible for stateful components such as databases. This choice prevents the consistency problems (and the corresponding complexity of potential solutions) that would arise if database were distributed to edge nodes. Moreover, this decision is aligned with the limitations of computational resources of edge nodes, as the deployment of databases to the edge would be unrealistic for large scale applications.
