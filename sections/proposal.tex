...
\begin{center}
	\begin{table}[th]
		...
		\begin{tabular}{ c c c c c }
			...
			\midrule
						Cloud	& ---	& ---				& \textbullet			& \textbullet\\
			\textbf{PP}-Edge	& ---	& ---				& \textbullet			& \textbullet\\
			\textbf{PR}-Edge	& ---	&\textbullet	& \textbullet			& \textbullet\\
			\textbf{RP}-Edge	& \textbullet	& ---	& \textbullet			& \textbullet\\
			\textbf{RR}-Edge	& \textbullet	& \textbullet	& \textbullet	& \textbullet\\
			\bottomrule
		\end{tabular}
	\end{table}
\end{center}
...
--- Cloud: services hosted in the cloud are, in most of the cases, reachable by means of a well-known Internet name (therefore, no awareness phase is needed). Assets used by these services are preliminarily deployed to cloud servers (thus, no acquisition is needed). Finally, cloud's elasticity mechanisms take care of the (de)allocation of infrastructure or platform resources according to a service level agreement (SLA). The later corresponds to A3-E's allocation phase. Finally, cloud services are invoked by clients (engagement phases). 
...
Next, the four A3-E phases are further described and mapped to the requirements elicited in  Section~\ref{sec:requirements}. 
