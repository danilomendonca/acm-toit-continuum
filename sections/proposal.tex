...
Such an architecture is already employed by multiple cloud providers\footnote{\url{https://blog.zhaw.ch/icclab/faas-function-hosting-services-and-their-technical-characteristics}}; its adoption by edge providers would have the following benefits: 1) \textit{scalability}, with the delegation of resource management; 2) \textit{efficiency}, with the provisioning of functions on demand without pre-allocation of computational resources; and 3) \textit{portability}, as functions are stateless and may be implemented in different languages, including those supported by mobile platforms. The serverless architecture represents the latest step in the recent shift of enterprise application architectures to containers and microservices~\cite{baldini2017serverless}, which also allows the A3-E model to overcome the heterogeneity of the continuum by adhering to service-oriented principles. 
...
The \textit{engagement} phase models the last stage of interaction between client applications and  service providers in the continuum. Throughout this phase, clients engage with (micro)\textit{services} --- implemented through functions --- by means of a request/response protocol (e.g., RESTful services over HTTP). In particular, to enforce a common interface between clients and heterogeneous providers, mobile devices should also expose offered functionality as local services.

At this stage of A3-E's process, the application request has been handled and parsed by the client-side middleware (CSM). Also, one or more domains must have setup and allocated the requested service for execution and the CSM must have selected a specific domain among all providing the same service. Finally, client's request are sent, processed, received, and returned to the application.
...
From the domain's perspective, the allocation phase models the placement of services among its pool of runtime resources. In specific, it models the allocation of stateless functions to the pool of servers, virtual machines, and containers managed by a domain. 
...
This phase is realized by the domain-side middleware (DSM) with a self-management loop~\cite{kephart2003vision} (Fig.~\ref{fig:service-allocation-loop}) in which functions corresponding to provided services are instantiated according to: (i) the monitored QoS (e.g., the availability of each service); (ii) the Service-Level Agreement (SLA); and (iii) the availability of computational resources. 
...
As an example, in the scenario introduced in Sec.~\ref{sec:continuum-scenario}, the AV application should have priority over non-critical applications such as an AR application for guiding tourists~\cite{GarrigaMendonca2017}. In the course of rush hours, edge services could become unavailable for the AR application, during which they would rely on local services or cloud-based services. In all other moments, the AR would be able to rely on edge-based services. 
...
Analogously to the domain-side, the CSM manages allocation with a self-management loop (Fig.~\ref{fig:domain-selection-loop}), this time by checking QoS levels of services from each available domain and deciding for the alternative that best satisfies the client's requirements. 
