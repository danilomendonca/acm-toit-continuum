...
The A3-E process is composed of four phases. Each phase encompasses activities -- performed by the client-side and domain-side middlewaress -- that take care of specific concerns in the interaction among client applications and the compute continuum. 

In order to overcome the heterogeneity of the continuum (Req. \textbf{R1.1}), A3-E adheres to the use of service-orientation in which clients engage with remove services by means of a request/response protocol (e.g., RESTful services over HTTP). Furthermore, A3-E process provides flexibility (Req. \textbf{R1.2}) with respect to its phases in order to addresses the particularities of the continuum constituents. In specific, the diagram in Fig.~\ref{fig:A3-E-model} represents an instance with the four phases. Later on, other possible instances of the A3-E model are mapped to distinct scenarios of the compute continuum.
...
The allocation phase has the following purposes: 1) to enable the efficient (Req. \textbf{R2.1}) and automated (Req. \textbf{R4.1}) allocation of domains' computational resources; and to enable clients to choose the best candidate among different available domains (Req. \textbf{R3.2}).
...
\begin{figure}[tbp]
	...
	\captionsetup[subfigure]{width=0.4\textwidth}	
	\hfill
	\subfloat[Domain allocation control loop; clients must monitor available domains and select the one that best satisfies its requirements.\label{fig:domain-selection-loop}] {\includegraphics[width=0.4\textwidth]{figs/domain-selection-loop}}
	\hfill\null
...
The Acquisition phase has the following purposes: 1) to enable the efficient (Req. \textbf{R2.2}) and automated (Req. \textbf{R4.2}) download and installation of service artifacts; and 2) to enable clients to identify, among the available domains, those compatible with their requirements.

From the domains perspective, A3-E furthers improve the efficiency of resource usage with the dynamic acquisition of service artifacts. In specific, this phase is modeled by the download and installation of artifacts upon identification of requirements from a given application. In case the service has already been acquired or as the acquisition finishes, clients should add that domain to their list of available domains. From the clients perspective, acquisition is modeled by the addition of a domain to a list of available domains.
...
Finally, the Awareness phase has the following purposes: 1) to enable a domain to pro-actively initialize other phases (namely acquisition and allocation) based on its awareness about application requirements hosted by clients in the domain coverage area (Req. \textbf{R3.3}); and 2) to enable clients to discover domains address (Reqs \textbf{R3.1}) so that domains that are not identified through well-known Internet names can be found and consumed in a opportunistic way (Req. \textbf{R3.2}).

From the domains perspective, the awareness of clients presence in their coverage area allows a proactive download and installation of services artifacts (acquisition phase) and/or the allocation of services (allocation phase) potentially before a first request to that service arrives, alleviating the delay introduced by services setup.  From the clients perspective, the awareness phase increases the range of domains from the continuum that can be used to satisfy their requirements.
...
\item \textit{Deallocation policy}
