...
Fig.~\ref{fig:Continuum-arch} illustrates the architecture of a mobile application designed to work with both the continuum and conventional cloud services. The client-side application consists of (client-side) business logic and user interfaces. Stateful server-side business logic and persistence (if any) are provided by conventional cloud services, i.e., Cloud IaaS. Finally, the application relies on the continuum for stateless computation, which is provided by microservices implemented as stateless function(s) and their static dependencies (if any). We consider conventional cloud services to be outside of the continuum because they can host virtually any type of application, apart from those that are continuum-compliant.
