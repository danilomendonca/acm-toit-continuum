...
%In order to overcome the heterogeneity of the continuum, A3-E adheres to the use of service-orientation in which clients engage with remove services by means of a request/response protocol (e.g., RESTful services over HTTP). 
...
The allocation phase has the following purposes: 1) to enable the efficient (Req. \textbf{R2.1}) and automated (Req. \textbf{R4.1}) allocation of domains' computational resources; and to enable clients to choose the best candidate among different available domains (Req. \textbf{R3.1}).
...
The Acquisition phase has the following purposes: 1) to enable the efficient (Req. \textbf{R2.2}) and automated (Req. \textbf{R4.2}) download and installation of service artifacts; and 2) to enable clients to identify, among the known domains, those compatible with their requirements.
...
From the clients perspective, acquisition is modeled by the addition of a domain to a list of available domains.
...
Finally, the Awareness phase has the following purposes: 1) to enable a domain to pro-actively initialize other phases (namely acquisition and allocation) based on its awareness about application requirements hosted by clients in the domain coverage area (Req. \textbf{R3.2}); and 2) to enable clients to discover the address of domains that are not identified through well-known Internet names (Req.~\textbf{R1.2}).

% can be found and consumed in a opportunistic way (Req. \textbf{R3.2}).
