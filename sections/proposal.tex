...
Given the heterogeneity of the continuum, distinct domains rely on different technologies to expose offered functionalities (e.g., through RESTful endpoints or WebSockets). Particularly, a mobile domain also expose its functionality --- also implemented as statele(s) functions --- by means of an interface (e.g., by means of event listener). For the sake of simplicity, the term microservice is also employed with mobile domains.
