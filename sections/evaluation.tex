...
\gio{In the following we have the experimental setup and thus it could be inserted in a dedicated section. Herein it would be better to have a brief introduction about what we wanted to demonstrate, which experiments we used and which results we obtained. In general I think that this section it is the one that needs more refinements (maybe more than refinements) since it seems a bit detached from the proposal. We have different set of examples what they demonstrate? It is written but in very informal terms such as "In our second set of experiments we wanted to evaluate A3-E as an enabler of the computation continuum.". I think that the first set of example is a preliminary study from the domain perspective to study how the latency changes with a varying workload. Of course the  the mobile domain here is omitted. The second set is about evaluating the domains from the perspective of the client in terms of battery and latency. Here we show that edge is the best since it minimizes the battery consumption and latency . Finally the last example evaluate the whole approach with a client that interacts we different domains that change their state (availability) according to a probabilist model. This is the most important experiment for me. Here we show that i) the model works since we can select domains at runtime ii) we improve availability since, by including the mobile, we reach 100\% of availability (btw how does that 93\% without mobile comes from? Shouldn't it be 86\% that is the availability of client internet/wireless connection? About this shouldn't be better to speak of connection availability and not internet availability since we are considering the local edge (Wi-Fi))}.

\subsection{Domains Setup}

%\subsubsection{Domains}

We deployed a set of domains that materialize the computing continuum to evaluate our approach --- as shown in Table~\ref{tab:domain-exp-config}. The sample application is an image recognition application, in which users can capture an image and invoke microservices in the continuum to perform the \textit{extraction of features} and \textit{detection of objects} in the image.
...
The main goal of this setup was not to compare traditional cloud services against a serverless solution, but to demonstrate that A3-E in the continuum could outperform cloud-only solutions under the tested circumstances and requirements.
...
\subsection{Evaluation of A3-E in the Continuum} 
 ...
 %The second set is about evaluating the domains from the perspective of the client in terms of battery and latency. Here we show that edge is the best since it minimizes the battery consumption and latency
In our second set of experiments the main goal is to evaluate the continuum from the client's perspective, in terms of battery consumption and latency. A mobile device was setup to host the A3-E middleware, which was in charge of dynamically selecting the best domain for a specific execution, based on the perceived QoS. 
Once again our sample application was the image recognition one with feature extraction and matching provided along the continuum. In this case we had three domains: mobile, local-edge, and Cloud-FaaS -- all according the specifications in Table~\ref{tab:domain-exp-config}. The experiment featured four different scenarios: the first three consider one domain each, and the last one (\textit{all-domains}) combines the previous ones to form the continuum. Details on how we modeled the availability of each domain in the \textit{all-domains} scenario are discussed in Section~\ref{sub:domain-selection}.
...
\begin{figure}[htb]
	...
	\subfloat[Total execution time (seconds)\label{fig:total-exec-a3e}] {\includegraphics[width=0.33\textwidth]{figs/total-exec-time-A3E}}
	\captionsetup[subfigure]{width=0.33\textwidth}
	\subfloat[Battery consumption (\%)\label{fig:battery-a3e}] {\includegraphics[width=0.33\textwidth]{figs/battery-consumption-A3E}}
	\captionsetup[subfigure]{width=0.33\textwidth}
	\subfloat[Execution time per call (milliseconds)\label{fig:time-per-call-a3e}] {\includegraphics[width=0.33\textwidth]{figs/time-per-call-A3E}}
...
These experiments tell us that the total execution time, when using only the cloud, was two times higher than when using the continuum (\texttt{all-domains}). This is because the requests were performed in cascade, and given the higher latency per call in the cloud, the total time increases accordingly. Clearly, using A3-E to switch to edge domains when possible substantially reduces the latency and improves the perceived QoS.
...
Finally we evaluated the capability of the A3-E prototype to select the best domain given low latency and  high computational power requirements. In this experiment we used the three domains together to form the continuum (\textit{all-domains} scenario). To do that, we probabilistically simulated the availabilities of the three domains. 

A3-E was configured to ping for domain availability, as well as perceived QoS, every two seconds ($u = 2$). We consider that the cloud domain could be unavailable mainly due to absence of mobile network's coverage, since downtimes of cloud services are minimal~\cite{garcia2017bandwidth}. To simulate this, the average unavailability of the coverage was set to once every $15$ minutes. Using a geometric distribution, the probability of being unavailable during a ping (\textit{connectionOnToOff}) was $0.022$. Every time the cloud was unavailable, the average time for it to become available again was $2$ minutes, i.e., the probability  \textit{connectionOffToOn} was $0.166$. Overall, this gives us a probability \textit{connectionAvailable} $=15/(15+2)=0.88$.
...
The rationale for the edge domain is analogous, yet with a higher probability of it being unavailable --- e.g., due to the lack of physical proximity with the edge servers. In this case, the edge is unavailable once every $10$ minutes, i.e., the probability was \textit{edgeOnToOff}  $ = 0.33$. Once unavailable, we considered that it would take an average of $5$ minutes for it to become available again, i.e. the probability \textit{edgeOffToOn} was $0.066$. Therefore, the probability \textit{edgeAvailable} $=10/(10+5)=0.66$. If we consider that edge nodes are only reachable within network coverage, then the probability \textit{edgeAvailableWithConnection}$=0.88*0.66=0.58$.
...
Figure~\ref{fig:all-domains} shows detailed results for the \texttt{all-domains} scenario with regards to availability and consistency. Given the availability probabilities for edge and cloud discussed above, the experiment shown (see Fig.~\ref{fig:availability-all-domains}) an average of $93\%$ availability without considering the mobile domain ($5\%$ and $35\%$ improvement regarding cloud and edge respectively), and $100\%$ availability when considering also the mobile domain (which is always available). 

Figure~\ref{fig:calls-per-domain-a3e} shows the average number of calls that were processed by each domain in the continuum: $65$\% by \texttt{local-edge}, followed by \texttt{cloud-FaaS} ($30$\%) and \texttt{mobile} ($7$\%). In terms of consistency, given that all requests were served, on average $70\%$ of the requests perceived low latency, whilst the $30\%$ that relied on the cloud perceived a degradation in the QoS but still were processed successfully. 
 
\begin{figure}[htb]
	\raggedright

	\captionsetup[subfigure]{width=0.45\textwidth}
	\subfloat[Availability per domain (\%)\label{fig:availability-all-domains}] 
	{\includegraphics[width=0.45\textwidth]{figs/availability-all-domains}}
	\captionsetup[subfigure]{width=0.45\textwidth}
	\subfloat[Number of Calls per domain\label{fig:calls-per-domain-a3e}] {\includegraphics[width=0.45\textwidth]{figs/calls-per-domain-A3E}}
	
	\caption{All-domains scenario results} \label{fig:all-domains}
\end{figure}

Conclusively, the experiment shown that A3-E is capable of performing domain selection and computation distribution at runtime. A3-E also improved availability up to 100\% during the experiment, given that the mobile domain is always available, but only selected to serve 7\% of the requests.
In the context of the \texttt{all-domains} scenario, this reflects the underlying idea of the computation continuum: exploit the edge domains as much as possible (by giving them high scores in Formula~\ref{eq:smart}). This will lead to better balance among computation time, latency and resource consumption. Switch to the cloud, or to the local device, only when necessary. Note that in this experiment the computation in the mobile device was configured with the lowest possible score, thus the cloud was selected as the first alternative to edge domains. This behavior can be tuned by adjusting application requirements and domain parameters discussed in Section~\ref{sec:implementation}.
